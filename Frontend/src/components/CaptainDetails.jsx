import React from 'react'

const CaptainDetails = () => {
  return (
    <div>
       <div className=' px-5 relative py-5   flex justify-between items-center'>
        <div className='flex  justify-center items-center gap-5'> 
            <img className='  rounded-full w-16 object-cover h-16  ' src='data:image/jpeg;base64,' alt=''/>
            <h3 className=' font-semibold text-right text-xl'>Ashish</h3>
        </div>
        <div className='px-3 '>
            <h4 className=' font-bold text-right text-xl'>₹295.20</h4>
            <h4 className=' font-normal text-left text-gray-600 '>Earned</h4>
        </div>
    </div>


    <div className=' bg-gray-100 mx-4 p-2 flex items-center justify-evenly rounded-xl'>
      <div className=' flex flex-col justify-center items-center'>
        <div className=' flex flex-col justify-center items-center'>
          <h3 className=' text-3xl font-medium'><i className="ri-timer-2-line"></i></h3>
          <h2 className='text-2xl font-medium'>10.2</h2>
        </div>
        <h4 className=' text-gray-600 text-sm'>Hours Online</h4>
      </div>
      
      <div className=' flex flex-col justify-center items-center'>
        <div className=' flex flex-col justify-center items-center'>
          <h3 className=' text-3xl font-medium'><i className="ri-speed-up-line"></i></h3>
          <h2 className='text-2xl font-medium'>10.2</h2>
        </div>
        <h4 className=' text-gray-600 text-sm'>Hours Online</h4>
      </div>

      <div className=' flex flex-col justify-center items-center'>
        <div className=' flex flex-col justify-center items-center'>
          <h3 className=' text-3xl font-medium'><i className="ri-booklet-line"></i></h3>
          <h2 className='text-2xl font-medium'>10.2</h2>
        </div>
        <h4 className=' text-gray-600 text-sm'>Hours Online</h4>
      </div>
    </div>
    </div>
  )
}

export default CaptainDetails
